    ...
    <div class="container mt-5">
        ...
        <div class="card shadow-sm p-4">
            ...
            <div class="parallelogram">
                @foreach ([
                    ...
                    3 => ['label' => 'Script Segments', 'key' => 'segmentation', 'path' => 'segments_path'],
                    ...
                    <div class="stage {{ $orderStatus->stage > $stage ? 'stage-completed' : '' }}">
                        ...
                        @if (isset($orderStatus->{$data['path']}))
                            <a href="{{ route('order.view-file', ['id' => $order->id, 'path' => $data['path']]) }}" class="btn btn-info btn-sm">View File</a>
                            <form action="{{ route('order.update-status', ['id' => $order->id, 'key' => $data['key'], 'path' => $data['path']]) }}" method="POST" class="mt-2">
                                @csrf
                                <div class="btn-group d-flex">
                                    <button name="action" value="approve" class="btn btn-success btn-sm" {{ $approved[$data['key']] ? 'disabled' : '' }}>Approve</button>
                                    <button name="action" value="disapprove" class="btn btn-danger btn-sm" {{ $orderStatus->stage > $stage ? 'disabled' : '' }}>Disapprove</button>
                                </div>
                            </form>
                        @endif
                    </div>
        ...
        <div class="card shadow-sm p-4 mt-4">
            ...
            <ul class="list-group list-group-flush">
                <li class="list-group-item"><strong>Expected Delivery:</strong> {{ $order->num_modules ? $order->created_at->copy()->addDays($order->num_modules * 2)->format('d M Y') : 'N/A' }}</li>
                ...
                <li class="list-group-item"><strong>Expected Duration:</strong> {{ $order->num_modules ? $order->num_modules * 3 . ' min' : 'N/A' }}</li>
                ...
                <li class="list-group-item"><strong>Brand Color:</strong> <span style="width: 20px; height: 20px; background-color: {{ $order->brand_color }}; display: inline-block;"></span> {{ $order->brand_color }}</li>
                ...
                <li class="list-group-item"><strong>Cost:</strong> ₹{{ $data['expected_cost'] ?? '0.00' }}</li>
            </ul>
        </div>
    </div>
